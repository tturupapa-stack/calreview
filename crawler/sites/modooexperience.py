...
def _crawl_category(cat_id: str, c_type: str, c_cat: str, max_pages: int = 20) -> List[Campaign]:
    ...
    # url pattern: campaign_list.php?category_id=...&page=...
        ...
        try:
            ...
            if not cards:
                ...
                if empty_page_count >= 2:  # 연속 2페이지가 비어있으면 종료
                    logger.debug(f"[모두의체험단] {c_cat} - 연속 빈 페이지로 크롤링 종료 (page {page})")
                    break
                continue
            else:
                empty_page_count = 0  # 게시물이 있으면 카운트 리셋
            
            logger.info(f"[모두의체험단] {c_cat} ({cat_id}) Page:{page} - {len(cards)}개 발견")
            ...
            logger.error(f"[모두의체험단] 요청 실패 ({cat_id}) Page {page}: {e}")
            empty_page_count += 1
