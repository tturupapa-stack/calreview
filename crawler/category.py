...
# 표준 카테고리 목록 (소스 사이트에서 직접 사용하면 신뢰)
STANDARD_CATEGORIES = {
    "맛집", "뷰티", "여행", "생활", "식품", "패션", "디지털",
    "문화", "반려동물", "유아동", "도서", "재택", "배송", "숙박", "카페"
}

# 소스 사이트 카테고리 → 표준 카테고리 매핑
# 소스 사이트에서 특정 카테고리를 제공하면 이 매핑을 따름
CATEGORY_MAPPING = {
    # 맛집 관련
    "맛집": "맛집",
    "카페": "맛집",  # 카페도 맛집으로 통합
    ...
    # 배송형 (타입이 아닌 카테고리로 사용된 경우)
    "배송": "배송",
    "제품": "배송",  # 일반적으로 제품 = 배송형
}

# 키워드 기반 분류가 필요한 "애매한" 카테고리
AMBIGUOUS_CATEGORIES = {"기타", "방문", "지역", ""}
...
def normalize_category(site_name: str, raw_category: Optional[str], title: str) -> str:
    ...
    우선순위:
    1. 소스 사이트의 명확한 카테고리가 있으면 매핑 사용
    2. 애매한 카테고리(기타, 방문 등)이거나 없으면 키워드 기반 분류
    ...
    # === 1단계: 소스 사이트 카테고리가 명확하면 매핑 적용 ===
    if cat and cat not in AMBIGUOUS_CATEGORIES:
        # 정확한 매핑 먼저 확인
        if cat in CATEGORY_MAPPING:
            mapped = CATEGORY_MAPPING[cat]
            # modan의 "제품"은 방문형 매장이므로 배송이 아님
            if cat == "제품" and site_name == "modan":
                pass  # 키워드 기반으로 넘어감
            else:
                return mapped
        ...
        for key, value in CATEGORY_MAPPING.items():
            if key in cat:
                if key == "제품" and site_name == "modan":
                    continue
                return value

    # === 2단계: 키워드 기반 분류 (애매한 카테고리이거나 카테고리 없음) ===
